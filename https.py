...
#####################################################
# To use HTTP port at 80 or HTTPS port at 443 we need to be superuser
# To be superuser we need to use sudo command to run python
# However, whenever we become superuser, the path settings gets initialized to sudo
# The sudo site-library does not contain many of the necessary libraries
# Therefore, we change the site-library for sudo by inserting the default library
###########

import sys
sys.path.insert(0,'/home/lombard/.local/lib/python3.8/site-packages')
...
def index():
    return(render_template('sampleGET.php'))

@app.route('/embedding',methods = ['GET'])
def embedding():
    document = request.args.get('document')
    print(document)
    return("Hello AKT's Embedding code"+document)
